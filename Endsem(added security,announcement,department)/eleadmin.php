<?php
session_start();
require_once "sendpulse/src/ApiInterface.php";
require_once "sendpulse/src/ApiClient.php";
require_once "sendpulse/src/Storage/FileStorage.php";
require_once "sendpulse/src/Storage/TokenStorageInterface.php";
require_once "sendpulse/src/Storage/SessionStorage.php";
require_once "sendpulse/src/Storage/MemcachedStorage.php";
require_once "sendpulse/src/Storage/MemcacheStorage.php";
define('API_USER_ID', 'd7036045e747d7471431f16dbf0aef44');
define('API_SECRET', '897e86764bfa71e7571ae50e8420bddb');
	...
	$('#insert').click(function(){
		...
		else
		{
            ...
            if (typeof ($("#image")[0].files) != "undefined") {
				...
				if(size>999.31){
				alert("Image Size is more than 999kb. Please Upload Image less Than 999kb.");
			}
			else
			{
...
<?php
	...
	<thead>
			  ...
			  ";
				...
				<td><select name='year' style='width:65px;border:solid gray;border-radius:10px;' required>
				   ...
				   <td>
						<input type='text' maxlength='3' style='width:50px;border:solid gray;border-radius:10px;'>
				   </td>
   ...
   if(isset($_POST['viewres']))
   {
	   ...
	   echo "<script>getchart('$tab');</script>
		   ...
		   <input class='button' type='submit' value='View Other Results' name='result' style='margin-left:50px;'></div></div></div></form>
			...
			{
				echo "Post $pos Has More Than One Winner<br>";
				while($res=mysqli_fetch_array($result))
			   {
				$regnum=$res['regnum'];
				$pname1=$res['nname'];
			   echo "<p><center><b>$regnum : $pname1<b></cenrer><p>";
			   }
			   echo "</div></div></div>
			</div>
			</div>
   ...
   if(isset($_POST['announce']))
   {
	   ...
	   foreach($tables as $table)
	   {
		   ...
		   $sql="SELECT regnum,nname from $table where IT+EEE+EC+ME+MT+CH+CS+CIVIL = (select MAX(IT+EEE+EC+ME+MT+CH+CS+CIVIL) from $table) ";
			$result=mysqli_query($con,$sql);
			if($result->num_rows == 1)
			{
			while($res=mysqli_fetch_array($result))
		   ...
		   $msg= $msg . "<p><b>\"". $table1 ." -> ". $pname1 . "(". $regnum .")\"<b><p>";
		   
			}
	   }
	   $sql = "select * from users";
	   $newres = mysqli_query($con,$sql);
	  while($newr = mysqli_fetch_array($newres))
	   {
		...
		'from' => array(
        'name' => 'OVS_ADMIN',
        'email' => '15IT215.lakshmish@nitk.edu.in',
		),
		'to' => array(
        array(
            'name' => 'OVS_USER',
...
var x = setInterval(function() {
    ...
    if (distance < 0) {
        ...
        document.getElementById("demo1").innerHTML = "<input class='button' type='submit' value='Announce Result' name='announce' style='margin-left:320px;width:250px;margin-top:25px;'>";
		}
}, 1000);
</script>";
    }
